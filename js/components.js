...
function createVideoCard(video, layout = 'grid') {
    ...
    <a href="watch.html?v=${video.id}" style="display: block; text-decoration: none; color: inherit;">
      <div class="video-thumbnail-container">
        <img
          ...
          loading="lazy"
          onerror="this.onerror=null;this.src='assets/thumb-placeholder.jpg';"
        >
        <button class="video-report-btn" type="button" data-video-id="${video.id}" aria-label="Report video" title="Report">
          ${reportIcon}
        </button>
      ...
      <div class="video-info">
        <div class="video-avatar">
          ${renderAvatarWithBadge(profile, { imgClass: 'avatar' })}
        </div>
        <div class="video-details">
  ...
  const reportBtn = card.querySelector('.video-report-btn');
  if (reportBtn) {
    reportBtn.addEventListener('click', async (event) => {
      ...
      const reason = await showPrompt('Report reason', {
        title: 'Report video',
        placeholder: 'spam, copyright, other',
        confirmText: 'Continue'
      });
      if (reason === null) return;
      const trimmed = reason.trim();
      if (!trimmed) {
        showNotification('Report canceled', 'info');
        return;
      }
      const details = await showPrompt('Optional details', {
      ...
      try {
        await reportVideo(video.id, trimmed.toLowerCase(), detailsValue.trim());
        showNotification('Report submitted. Thanks for letting us know.', 'success');
      } catch (error) {
